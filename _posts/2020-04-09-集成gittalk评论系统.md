---
layout: post
title: "10分钟搞定gittalk评论系统"
subtitle: "Jekyll集成gittalk评论系统教程"
author: "Ryan"
tags:
  - 博客
---
# 集成gittalk评论系统
由于Disqus需要翻墙才能访问，在国内使用起来很不方便，经常会出现无法加载的情况，而其他诸多评论系统要么收费，要么使用起来比较麻烦。对于一般的需求，推荐使用gittalk，功能稳定，支持markdown格式的评论，对于一般的个人博客来讲，完全够用。

以下列出了集成gittalk所需的步骤，按本文中的步骤来，只需要10分钟就能搞定！效果图如下：

![demo](/img/gittalk/demo.png)

## 1. 申请Github OAuth Application

Github头像下拉菜单 > Settings > 左边Developer settings下的OAuth Apps > Register a new application，填写相关信息：

![oauth-app](/img/gittalk/oauth-app.png)

## 2. 在jekyll博客添加gitalk

这里我使用的是开源博客项目来搭建自己的博客，如果你想搭建自己的github博客，可以参考博客搭建教程：
...
![blog-dir](/img/gittalk/blog-dir.png)

你需要打开`_layouts`目录下的`post.html`文件，并在代码顶部加入如下代码：
...
在根目录下的css目录中，创建gittalk.css文件，文件内容如下：
...
在根目录下的js目录中，创建gittalk.min.js文件，文件内容如下：
...
在include文件夹中，创建comments.html文件，内容如下：
...
将gittalk部分的代码添加到disqus和网易云跟帖下面即可：
...
owner: Ryan-Shz
repo: *****.github.io
clientID: ****2f83c78c796c3b70
clientSecret: *******50efca9134193199ac649bc86
admin: Ryan-Shz
```

## 8. 打开github博客项目的issues功能

github项目 > Settings > Features > Issues打勾即可